...
const LogTerminal = ({ isActive }: LogTerminalProps) => {
  const [logs, setLogs] = useState<LogEntry[]>(sampleLogs.slice(0, 3));
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (idx < sampleLogs.length) {
        setLogs((prev) => [...prev, sampleLogs[idx]]);
        idx++;
      } else {
        // Loop with new IDs
        const base = sampleLogs[Math.floor(Math.random() * sampleLogs.length)];
        const now = new Date();
        const time = `${now.getHours().toString().padStart(2, "0")}:${now.getMinutes().toString().padStart(2, "0")}:${now.getSeconds().toString().padStart(2, "0")}`;
        setLogs((prev) => [...prev.slice(-30), { ...base, id: Date.now(), time }]);
        idx++;
      }
    }, 2000);
